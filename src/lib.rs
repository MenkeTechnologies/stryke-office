...
/// Count filled (non-blank) and blank cells per column — data-completeness
/// profiling (pandas `count` plus the null tally). opts: path, sheet, header.
/// Returns `{ sheet, rows, columns: [{ name, count, blank }] }` where `count` is
/// the non-blank cells and `blank` the empties, over the data rows.
fn op_sheet_count(opts: Value) -> Result<Value> {
    let path = req_str(&opts, "path")?;
    let read = op_sheet_read(json!({ "path": path }))?;
    let sheets = read
        .get("sheets")
    ...
    for c in 0..ncols {
        ...
        let cname = header_row
            ...
            .unwrap_or_else(|| format!("Col{}", c + 1));
        columns
            .push(json!({ "name": cname, "count": filled, "blank": data.len() as u64 - filled }));
    }
    Ok(json!({ "sheet": name, "rows": data.len(), "columns": columns }))
}
...
/// Append a "percent of group total" column: each value as a percentage of the
/// sum of its group (pandas `groupby(group)[value].transform(lambda s: s/s.sum())
/// * 100`). The group-aware counterpart to `sheet_pct` (percent of the whole
/// column). Rows are grouped by the `group` column's value (no sorting required).
/// opts: path, output, group => column name/index (required), value => numeric
/// column name/index (required), into => header (default "{value}_grouppct"),
/// decimals, sheet, header, format. Non-numeric cells and zero-sum groups get a
/// blank. Returns `{ ok, path, column }`.
fn op_sheet_group_pct(opts: Value) -> Result<Value> {
    let path = req_str(&opts, "path")?;
    let output = req_str(&opts, "output")?.to_string();
    ...
    let into = opts
        ...
        .and_then(Value::as_str)
        .map(str::to_string)
        .unwrap_or_else(|| format!("{base}_grouppct"));
    let decimals = opts.get("decimals").and_then(Value::as_i64);

    let data_start = if header && !rows.is_empty() { 1 } else { 0 };
    let group_key = |row: &Value| -> String {
        row.as_array()
            .and_then(|a| a.get(gcol))
            .map(cell_to_string)
            .unwrap_or_default()
    };
    // First pass: total per group.
    ...
    for row in &rows[data_start..] {
        if let Some(x) = row
            ...
            .and_then(sheet_cell_num)
        {
            *totals.entry(group_key(row)).or_insert(0.0) += x;
        }
    }
    ...
    let new_rows: Vec<Value> = rows
        ...
        .map(|(i, row)| {
            ...
            let total = totals.get(&group_key(row)).copied().unwrap_or(0.0);
            let cell = match cells.get(vcol).and_then(sheet_cell_num) {
                Some(x) if total != 0.0 => {
...
/// Set (replace) the body lines of one slide — the body counterpart to
/// `slides_set_title`, for deck templating. The title (first text line) is
/// preserved. opts: path, output (default in place), slide => 1-based slide
/// number (required), body => array of body lines (required), format. Returns
/// `{ ok, path, slide }`.
fn op_slides_set_body(opts: Value) -> Result<Value> {
    let path = req_str(&opts, "path")?;
    let output = opts
        .get("output")
        .and_then(Value::as_str)
    ...
    let new_body = opts
        ...
        .filter(|v| v.is_array())
        .cloned()
        .ok_or_else(|| anyhow!("missing body (expected array of lines)"))?;
    ...
    let out_slides: Vec<Value> = existing
        ...
        .map(|(i, s)| {
            let text: Vec<String> = s
                ...
                .unwrap_or_default();
            let (title, cur_body) = text
                .split_first()
                .map(|(h, rest)| (h.clone(), rest.to_vec()))
                .unwrap_or_default();
            let body = if i + 1 == slide {
                new_body.clone()
            } else {
                json!(cur_body)
            };
...
export!(office__slides_set_body, op_slides_set_body);
...
export!(office__text_extract, op_text_extract);
...
export!(office__doc_footnotes, op_doc_footnotes);
...
export!(office__html_to_text, op_html_to_text);
...
export!(office__doc_summary, op_doc_summary);
export!(office__doc_extract, op_doc_extract);
...
export!(office__img_phash, op_img_phash);
...
export!(office__barcode_sheet, op_barcode_sheet);

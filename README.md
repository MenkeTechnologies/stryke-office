...
| `Office::sheet_mad($path, $column, %opts)` | `{ok, mad, median, n}` | median absolute deviation (robust spread); `scaled` ×1.4826 for normal consistency |
...
| `Office::sheet_drawdown($path, $column, %opts)` | `{ok, max_drawdown, peak, trough, n}` | maximum peak-to-trough decline of a value series (risk metric) |
...
| `Office::sheet_str_distance($path, $a, $b, $output, %opts)` | `{ok, path, column}` | append string similarity between two columns (fuzzy match); `metric` levenshtein/ratio |
...
| `Office::sheet_join($left, $right, $output, %opts)` | `{rows, matched}` | SQL JOIN two sheets on a key; `on`/`left_on`/`right_on`, `how` inner/left/right/outer/semi/anti |
...
| `Office::sheet_replace($path, $find, %opts)` | `{replaced}` | find/replace cell text (any format incl. csv); `ignore_case`/`whole`/`column`/`regex` (`$1` captures) |
...
| `Office::sheet_date_diff($path, $start, $end, $output, %opts)` | `{ok, path, column}` | append signed difference (`end − start`) between two ISO-date columns; `unit` days/weeks |
| `Office::sheet_networkdays($path, $start, $end, $output, %opts)` | `{ok, path, column}` | append business-day count between two date columns (Excel `NETWORKDAYS`); `holidays` list opt |
| `Office::sheet_date_add($path, $column, $amount, $output, %opts)` | `{ok, path, column}` | append an ISO-date column shifted by `amount` (negative ok); `unit` days/weeks/months (month-end clamped) |
...
Office::sheet_write("s.xlsx", [{
    ...
    properties => { title => "Q Report", author => "jacob", company => "MenkeTech" })
...
| `Office::r1c1_to_a1($r1c1, $anchor?)` | `{ r1c1, cell, row, col, row_absolute, col_absolute }` — inverse of `a1_to_r1c1`; bracketless part → `$`-locked A1 (`R3C2`→`$B$3`), bracketed/bare part resolves against the anchor (`R[1]C[-1]`@`C3`→`B4`) |
...
| `Office::a1_abs_of($row, $col, $col_abs?, $row_abs?)` | A1 string with `$` markers — inverse of `parse_a1_abs` (`$A$1`, `A$1`, `$A1`) |
...
| `Office::normalize_range("C3:A1")` | `{ range, start, end, rows, cols }` — canonicalize corner order to top-left:bottom-right (`C3:A1`→`A1:C3`), upper-cased; the form to compare/store a range by |
...
| `Office::range_to_absolute($range)` | lock both corners with `$` (range analog of `a1_abs_of`) — `range_to_absolute("A1:B2")` → `{ range:"$A$1:$B$2", … }` |
| `Office::range_to_relative($range)` | inverse: strip the `$` locks — `range_to_relative("$A$1:$B$2")` → `{ range:"A1:B2", … }` |
